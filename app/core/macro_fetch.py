...
def fetch_macro(db):
    """
    Fetch macro data, using Turso cache if fresh enough (< 1 hour).
    ...
    try:
        ...
        # If we have cached data less than 1 hour old, return it
        if cached and all(
            r.get("updated_at", "") > one_hour_ago
...
def _fetch_egx30():
    ...
    try:
        ...
        return {
            ...
            "direction": direction,
            "trend": "bullish" if (monthly_change and monthly_change > 2) else
                     "bearish" if (monthly_change and monthly_change < -2) else "sideways",
        }
    except Exception:
        return {"value": None, "change_pct": None, "direction": None}
...
def _fetch_usdegp():
    ...
    try:
        ...
        prev = float(df["close"].iloc[-2]) if len(df) > 1 else None
        change_pct = ((current - prev) / prev * 100) if prev else None
        direction = "up" if (change_pct and change_pct > 0.1) else \
                    "down" if (change_pct and change_pct < -0.1) else "stable"
        return {
            "value": round(current, 2),
            "change_pct": round(change_pct, 2) if change_pct else None,
            "direction": direction,
...
def _fetch_interest_rate(db):
    ...
    try:
        ...
        rate = float(row[0]) if row else 25.0
        return {
            "value": rate,
            "direction": "stable",
        }
    except Exception:
        return {"value": 25.0, "direction": "stable"}
...
def _format_cached(cached, db):
    ...
    if "egx30" in cached:
        ...
        direction = ("up" if (monthly and monthly > 2) else
                     "down" if (monthly and monthly < -2) else "stable")
        result["egx30"] = {
            "value": c["value"],
            "change_pct": c["change_pct"],
            "monthly_change_pct": monthly,
            "direction": direction,
            "trend": ("bullish" if (monthly and monthly > 2) else
                      "bearish" if (monthly and monthly < -2) else "sideways"),
        }
    else:
    ...
    if "usd_egp" in cached:
        ...
        result["usd_egp"] = {
            "value": c["value"],
            "change_pct": c["change_pct"],
            "direction": "up" if (c["change_pct"] and c["change_pct"] > 0.1) else
                         "down" if (c["change_pct"] and c["change_pct"] < -0.1) else "stable",
        }
    else:
