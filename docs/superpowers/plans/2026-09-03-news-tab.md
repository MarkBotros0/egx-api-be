...
- Produces:
  ...
  - `build_feed(fetched: dict, yours: list[str], now: datetime, dropped=()) -> dict` → `{your_stocks, market, coverage}`

> **Amended after Task 4's review.** `select_news_symbols` now returns a
> THREE-tuple `(yours, market_only, dropped)`. `dropped` lists the user's own
> symbols the cap excluded. `build_feed` takes it as a keyword argument and
> reports it as `coverage.symbols_over_cap`, so coverage stays assembled in
> ONE place. A dropped holding must never vanish silently — same rule as the
> dashboard's "82 stocks · 84 without a price feed".
